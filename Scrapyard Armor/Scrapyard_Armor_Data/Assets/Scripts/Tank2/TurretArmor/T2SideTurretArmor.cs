...
public class T2SideTurretArmor : MonoBehaviour {
	...
	void OnTriggerEnter2D (Collider2D other)
	{
		...
		{
			//hitAngle = Vector2.Angle (transform.position, T1Shell.transform.position);
			...
			//if (T1penetrationScript.Penetration >= lineOfSightArmor) {
				...
				//Debug.Log ("Penetration on Side Turret!", this.gameObject);
		//	} 
		//else if (T1penetrationScript.Penetration <= lineOfSightArmor) {
			//	Destroy (other.gameObject);
			//	Debug.Log ("Shot Bounced on Side Turret!", this.gameObject);
			//}
		}
	}
}
